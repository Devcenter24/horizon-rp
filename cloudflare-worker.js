...
async function handleGetSettings(env) {
  ...
  return json({
    ...
    preview: {
      game: s.preview_game,
      rolesCount: s.preview_roles_count,
      language: s.preview_language,
      availability: s.preview_availability,
    },
  });
}
...
async function handleUpdateSettings(request, env) {
  const body = await request.json();
  const { discordId, recrutementOuvert, maintenance, preview } = body;
  ...
  if (preview && typeof preview === "object") {
    if (preview.game) update.preview_game = preview.game;
    if (preview.rolesCount) update.preview_roles_count = preview.rolesCount;
    if (preview.language) update.preview_language = preview.language;
    if (preview.availability) update.preview_availability = preview.availability;
  }
