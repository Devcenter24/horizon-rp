...
Dans **OAuth2 → Redirects** de ton application Discord, l'URI doit être exactement (avec le slash final) :
```
https://hznrp.netlify.app/dashboard/
```

---

## Structure finale du site (à déployer sur Netlify)
...
Le fichier `_redirects` (sans extension) est lu automatiquement par Netlify
à la racine du site. Il se limite à rediriger toute URL inconnue vers la
page 404 — Netlify gère déjà nativement `/dashboard/`, `/recrutement/` et
`/mentions-legales/` via leurs dossiers, sans configuration supplémentaire.

⚠️ Une version précédente de ce fichier tentait de forcer ces URLs sans
slash final, ce qui créait un conflit de règles interrompant le chargement
du dashboard en cours de route. Ne pas réintroduire de règles de
réécriture (`200`) sur ces chemins sans tester soigneusement.

Les fichiers suivants ne vont PAS sur Netlify (ils tournent ailleurs) :
