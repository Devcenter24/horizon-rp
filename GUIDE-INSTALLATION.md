...
Le cog `cogs/recrutement.py` (fourni) tourne sur ton VPS, à côté de ton bot existant.

1. Copie `cogs/recrutement.py` dans le dossier `cogs/` de ton bot
2. Dans ton `config.py`, ajoute ces deux lignes (à côté de `TOKEN`) :
   ...
   ```
3. Charge le cog au démarrage du bot, là où tu charges tes autres extensions :
   ```python
...
- `cogs/recrutement.py` + `utils/cv2_helpers.py` -> ton VPS, à côté de ton bot
