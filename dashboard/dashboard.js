...
async function init() {
  ...
  if (code) {
    ...
    try {
      ...
      if (returnTo && returnTo !== "/horizon-rp/dashboard" && returnTo !== "/horizon-rp/dashboard/") {
...
function initSettingsForm(user) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      preview: {
        game: formData.get("previewGame"),
        rolesCount: formData.get("previewRolesCount"),
        language: formData.get("previewLanguage"),
        availability: formData.get("previewAvailability"),
      },
    };
...
async function loadSettingsIntoForm() {
  ...
  try {
    ...
    form.elements.previewGame.value = s.preview?.game || "";
    form.elements.previewRolesCount.value = s.preview?.rolesCount || "";
    form.elements.previewLanguage.value = s.preview?.language || "";
    form.elements.previewAvailability.value = s.preview?.availability || "";
  } catch {
    const feedback = document.querySelector("[data-settings-feedback]");
    if (feedback) {
